        ...
        db.run(
            ...
            function (err) {
                ...
                const tryFind = (attempts) => {
                    db.get(
                        "SELECT id, username, score_facil, score_medio, score_dificil, score_experto FROM users WHERE username = ?",
                        [username],
                        (err, row) => {
...
// ─── ACTUALIZAR PUNTUACIÓN (mínimo 0) ─────────────────────────────────────────
app.post('/api/update-score', (req, res) => {
    ...
    // Primero obtenemos el score actual para no bajar de 0
    db.get(`SELECT ${col} as current FROM users WHERE id = ?`, [userId], (err, row) => {
        ...
        const newScore = Math.max(0, current + points);   // ← nunca negativo
        const delta    = newScore - current;               // delta real aplicado
...
app.get('/api/leaderboard', (req, res) => {
    ...
    db.all(
        `SELECT username, ${col} as score FROM users WHERE ${col} > 0 ORDER BY ${col} DESC LIMIT 10`,
        [],
        (err, rows) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json(rows);
...
app.get('/api/nueva-secuencia', (req, res) => {
    const dificultad = req.query.dificultad || 'fácil';
    if (!DIFF_COL[dificultad])
        return res.status(400).json({ error: "Dificultad inválida" });

    db.get(
        "SELECT * FROM secuencias WHERE dificultad = ? ORDER BY RANDOM() LIMIT 1",
        [dificultad],
        (err, row) => {
